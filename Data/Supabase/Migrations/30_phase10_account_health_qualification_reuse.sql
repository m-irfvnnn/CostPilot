-- ============================================
-- CostPilot Phase 10 — Account health qualification reuse
-- ============================================
-- Avoids repeated scans of lead_current_qualification inside the health signal
-- assembly by reusing the latest persisted qualification evaluations directly.

create index if not exists qualification_evaluations_account_lead_latest_idx
    on public.qualification_evaluations (account_id, lead_id, evaluated_at desc, created_at desc);
...
latest_qualification as (
    ...
    where qe.account_id is not null
      and qe.lead_id is not null
...
qualification_rollup as (
    select
        ...
        count(*) filter (where q.sql_status = 'sales_ready')::integer as sales_ready_lead_count,
        count(*) filter (where q.pql_status = 'product_qualified')::integer as product_qualified_lead_count,
        count(*) filter (where q.pql_status = 'product_activated')::integer as product_activated_lead_count,
        max(q.priority_score)::integer as max_priority_score
...
top_lead as (
    select distinct on (q.account_id)
        q.account_id,
        q.lead_id as top_lead_id,
        q.priority_score as top_lead_priority_score,
        q.priority_tier as top_lead_priority_tier,
        q.sql_status as top_lead_sql_status,
        q.mql_status as top_lead_mql_status
    from latest_qualification q
    order by q.account_id, q.priority_score desc, q.evaluated_at desc, q.lead_id desc
),
sales_rollup as (
    select
        sa.account_id,
